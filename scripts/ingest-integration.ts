    ...
    for (const filePath of files) {
      try {
        ...
        for (const speech of speeches) {
          try {
            // Insert the speech
            const speechId = await repositories.speeches.insertSpeech(speech);
            ...
            for (let i = 0; i < speechChunks.length; i++) {
              ...
              await repositories.speeches.insertChunk(speechChunk);
            }
...
export async function mainWithRepository() {
  ...
  const { getRepos } = await import("../src/container");
  ...
  if (result.errors.length > 0) {
    ...
    const speechList = await repositories.speeches.list(1, 1);
